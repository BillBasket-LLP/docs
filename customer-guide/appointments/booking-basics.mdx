...
<Tabs>
  <Tab title="Windows PC">
    The **New booking** form — Service picked (with its duration shown
    right under it), start date/time with the end time and duration
    calculated automatically, then customer search, staff/provider picker,
    Notes, and a Repeat toggle for recurring bookings.

    <Frame>
      ![New booking form with a service selected and start/end time calculated automatically](/images/appointments/new-booking-form-windows.png)
    </Frame>
  </Tab>
  <Tab title="Phone">
    <Frame>
